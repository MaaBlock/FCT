...
namespace FCT
{
    ...
    void OldRenderGraph::addWindowResource(Window* wnd)
    {
    ...
    void OldRenderGraph::addPassDenpendency(const std::string& from, const std::string& to)
    {
        PassGraphEdge e;
    ...
    void OldRenderGraph::bindOutputImage(const std::string& name, std::string image, uint8_t slot)
    {
    ...
    void OldRenderGraph::bindTextureImage(std::string name, std::string image, uint32_t width, uint32_t height,
        Samples samples)
    {
        ImageResourceDesc& desc = m_imageResourceDescs[image];
        desc.width |= width;
    ...
    void OldRenderGraph::bindDepthStencil(const char* name, std::string image)
    {
        ImageResourceDesc& desc = m_imageResourceDescs[image];
    ...
    void OldRenderGraph::bindOutput(const char* name, Window* wnd, uint8_t slot)
    {
    ...
    void OldRenderGraph::checkAndUpdateResourceSizes()
    {
    ...
    void OldRenderGraph::generateImageGraph()
    {
    ...
    void OldRenderGraph::computeSize()
    {
    ...
    void OldRenderGraph::updateFrameIndices()
    {
    ...
    void OldRenderGraph::saveExecutionOrder(const std::vector<PassGraphType::vertex_descriptor>& sortedPasses,
    ...
    void OldRenderGraph::generateImageResource()
    {
        computeSize();
        for (const auto& entry : m_windowImageResources) {
            const std::string& imageName = entry.first;
    ...
    void OldRenderGraph::analyzeImplicitDependencies()
    {
    ...
    bool OldRenderGraph::isPassWritingToResource(const std::string& passName, const std::string& resourceName)
    {
    ...
    bool OldRenderGraph::isPassReadingFromResource(const std::string& passName, const std::string& resourceName)
    {
        auto it = m_passVertex.find(passName);
        if (it == m_passVertex.end()) {
            return false;
    ...
    void OldRenderGraph::execute(RHI::CommandBuffer* cmdBuf)
    {
    ...
    void OldRenderGraph::printExecutionOrder()
    {
    ...
    void OldRenderGraph::printResourceInfo()
    {
        fout << "\n渲染图资源信息:" << std::endl;
        ...
        std::vector<std::string> OldRenderGraph::getPassTargetToWnd(Window* wnd)
    {
    ...
    std::string OldRenderGraph::formatToString(Format format)
    {
        switch (format) {
    ...
    void OldRenderGraph::updateResource()
    {
        for (const auto& passEntry : m_passVertex) {
            PassGraphType::vertex_descriptor passVd = passEntry.second;
            PassGraphVertex& passData = m_passGraph[passVd];
    ...
    void OldRenderGraph::swapJobQueue()
    {
        for (auto& passEntry : m_passVertex)
        {
            PassGraphType::vertex_descriptor passVd = passEntry.second;
